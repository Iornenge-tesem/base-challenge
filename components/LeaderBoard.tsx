      ...
      <div className="space-y-3">
        {leaders.length === 0 ? (
          ...
          leaders.map((leader, index) => (
            ...
            >
              <div className="text-lg font-bold bg-gradient-to-r from-accent-green to-accent-green-dark bg-clip-text text-transparent w-6 text-center">
                {index + 1}
              </div>
              
              {typeof leader.avatar === 'string' && leader.avatar.startsWith('http') ? (
                <img
                  src={leader.avatar}
                ...
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-accent-green to-accent-green-dark flex items-center justify-center text-primary-dark-blue font-bold text-sm">
                  {leader.displayName?.[0]?.toUpperCase() || '?'}
                </div>
              )}
              
              <div className="flex-1 min-w-0">
                <div className="font-medium text-primary-dark-blue dark:text-primary-white truncate">
                  {leader.displayName}
                </div>
              </div>
